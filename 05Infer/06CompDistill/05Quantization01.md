...
当前的低比特量化方案面临着硬件效率方面的矛盾。为了保持模型精度，W4A4等方法必须对权重和激活同时应用分组量化，这导致在GEMM计算的主循环中，需要将INT4 Tensor Core产生的INT32部分和实时去量化为FP32。然而，在A100等数据中心GPU上，这种去量化操作只能在吞吐量极低的CUDA Core上执行。单次CUDA操作的成本相当于50次INT4 Tensor Core运算，由此产生的20%-90%额外开销完全抵消了4-bit计算带来的理论性能优势，最终导致W4A4在实际部署中甚至比更高精度的W8A8方案还要慢20-25%，形成了“降低比特精度反而拖慢推理速度”的问题。

因此，减少CUDA Core的开销对于在LLM服务中实现最佳吞吐量至关重要。

### 技术实现

![图7：QoQ算法和QServe系统](./images/Quantazation_Figure_7.png)

论文提出了算法-系统协同设计的解决方案(如图7)：在算法层面，设计了QoQ量化算法，采用创新的W4A8KV4混合精度配置，其中4-bit权重实现内存节省，8-bit激活确保计算稳定性，4-bit KV缓存缓解注意力带宽瓶颈；并提出渐进式分组量化，通过保护范围确保所有计算映射到INT8张量核心，以及SmoothAttention技术将Key矩阵的离群值平滑至可量化范围。在系统层面，开发了QServe推理系统，通过计算感知的权重重排优化内存访问模式，采用减法后乘的计算顺序将去量化移出主循环，并利用寄存器级并行最大化指令吞吐。最终，该方案在A100和L40S GPU上实现了相对于TensorRT-LLM最高3.5倍的吞吐量提升，首次在更低成本的GPU上达到了超越高端GPU的推理性能。
