...
## LLM.int8()量化
在 Transformer 模型中，前馈网络（FFN）和注意力投影层的矩阵乘法操作是内存消耗的主要开销来源，大概消耗了 95%的参数资源和 65%-85%的计算量。因此，降低这些线性层的内存占用是解决整个模型内存问题的关键。在现有的解决方案中，8-bit 量化是减少内存占用的直接方法（将模型权重从 2 字节降至 1 字节，理论上内存减半）。但存在一些局限：(1)之前的 8-bit 量化方法（如 Q8BERT）会导致模型性能（如准确率、困惑度）显著下降。(2)这些方法仅在小规模模型（< 350M 参数） 上被验证有效，例如 BERT。(3)这些方法量化后的模型通常需要量化感知训练或训练后微调来缓解性能损失，对于十亿级别以上的自回归模型（如 GPT），无损的 8 比特量化是一个未解决的挑战。
...
如图3a所示，在不同参数量的模型（125M, 1.3B, 2.7B, 6.7B, 13B）上，计算其“受影响层比例”和“受影响序列维度比例”，然后以模型规模为横轴，以这些比例为纵轴进行绘图。可发现，当模型规模达到6.7B参数时，“受影响层比例”从65%突然跃升至100%。同时，“受影响序列维度比例”也从35%急剧上升到75%。这种突然的转变与量化开始失效的点同时发生。
...
INT4量化的应用场景严格限定于推理阶段，因为训练所需的反向传播在4比特精度下会因梯度消失而完全失效。在推理部署中，具体策略的选择是一场在精度、速度与易用性之间的精妙权衡。如果需要做到极致压缩与最快速度，且硬件支持完整，可选择权重与激活值全量化，但须配备如FlatQuant这类能通过可学习变换重塑数据分布以应对异常值的先进算法来保障精度；如果优先考虑部署简便与高精度，则权重仅量化是理想的折衷方案，它能大幅减少存储占用且精度损失可控，但无法压缩激活值，加速效果有限；而比较普适的策略是混合精度量化，其通过对量化敏感的层保留高精度、对冗余层实施INT4量化，在效率与精度间寻求最佳平衡。

## 混合精度量化(以 QServe为例)

传统量化研究遵循着一条清晰的"比特递减"轨迹，在深入探索了LLM.int8的8-bit全量化精度保持和FlantQuant的4-bit权重极致压缩后，会考虑到一个直接的问题：能否为模型的不同组件量身定制最合适的精度？混合精度量化不追求在所有维度使用最低比特，而是根据模型不同组件的特性和硬件限制，智能分配不同的量化精度。24年的文献QServe提出的W4A8KV4（4-bit权重，8-bit激活，4-bit KV缓存）工作是这种思想的典型代表，前面所述的LLM.int8这项工作也应用了这一思想。

### 思路洞察

整个研究的起点来源于两个问题：(1)更低的精度（如 W4A4）在云端大模型推理中没有带来预期的加速，为什么？(2)对于云端推理，什么样的量化方案才是“最优解”？

对于问题1，当前的低比特量化方案面临着硬件效率方面的矛盾。直觉上，在英伟达A100/H100这类数据中心GPU上，INT4 Tensor Core的峰值理论算力是INT8 Tensor Core的2倍。从而，将模型量化为W4A4应该能带来最大的计算吞吐量。但是论文指出，当时最先进的W4A4服务系统（如Atom）在实际运行中，性能比TensorRT-LLM的W16A16（全精度）版本还要低20-25%。

这一问题的根本原因在于反量化开销。为了保持模型精度，W4A4等方法必须对权重和激活同时应用分组量化，这导致在GEMM计算的主循环中，需要将INT4 Tensor Core产生的INT32部分和实时去量化为FP32。然而，在A100等数据中心GPU上，这种去量化操作只能在吞吐量极低的CUDA Core上执行。单次CUDA操作的成本相当于50次INT4 Tensor Core运算，由此产生的20%-90%额外开销完全抵消了4-bit计算带来的理论性能优势，最终导致W4A4在实际部署中甚至比更高精度的W8A8方案还要慢20-25%，形成了“降低比特精度反而拖慢推理速度”的问题。

从而，减少CUDA Core的开销对于在LLM服务中实现最佳吞吐量至关重要。

对于问题2，其根本在于，在 GPU的GEMM主循环中，任何在慢速 CUDA 核心上进行的反量化操作，都会严重抵消低精度 Tensor Core 带来的理论算力优势。

论文中将现有的部署量化方案总结为三类：W4A16（分组量化）、W8A8（每个通道权重+每个token激活）、W4A4（分组量化）。在W4A16方法中，计算在FP16 Tensor Core上进行，但权重是INT4。因此，在GEMM核函数的主循环中，必须将INT4权重反量化为FP16。这个反量化操作在慢速的CUDA核心上执行，在A100上，一个FP32 CUDA核心操作的成本，相当于50个 INT4 Tensor Core操作。在W4A4方法中，常采用的分组量化会导致INT4 Tensor Core计算出的结果是INT32的部分和。在GEMM主循环中，必须将这些INT32部分和反量化为FP32，再进行累加。这一操作同样在慢速的CUDA核心上执行，完全主导了GEMM主循环的运行时间。在W8A8的方法中，其主循环反量化可完全运行在 INT8 Tensor Core上，计算效率高。但权重内存占用是W4的2倍。在内存带宽受限场景下，性能会劣于W4方案。

### 技术实现

![图7：QoQ算法和QServe系统](./images/Quantazation_Figure_7.png)

论文提出了算法-系统协同设计的方案(如图7)：在算法层面，设计了QoQ量化算法，采用W4A8KV4混合精度配置，其中4-bit权重实现内存节省，8-bit激活确保计算稳定性，4-bit KV缓存缓解注意力带宽瓶颈；提出渐进式分组量化，通过保护范围确保所有计算映射到INT8张量核心；应用SmoothAttention技术将Key矩阵的离群值平滑至可量化范围。在系统层面，开发了QServe推理系统，通过计算感知的权重重排优化内存访问模式，采用减法后乘的计算顺序将去量化移出主循环，并利用寄存器级并行最大化指令吞吐。最终，该方案在A100和L40S GPU上实现了相对于TensorRT-LLM最高3.5倍的吞吐量提升，首次在更低成本的GPU上达到了超越高端GPU的推理性能。
